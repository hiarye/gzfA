<template>
    <div>
        <Row :gutter="gutterNum" style="margin:0px">
...
export default {
    ...
    data () {
        return {
            currPro: '', // 形如{'110000': '北京市'}
            currCit: '', // 形如{'130100': '石家庄市'}
            currCou: '',
            currStr: '',
            provList: areaData[86], // 所有省份：{'110000': '北京市', '120000': '天津市', '130000': '河北省', ...}
            cityList: {}, // 对应省份的所有城市,形如 {'130100': '石家庄市', '130200': '唐山市', '130300': '秦皇岛市', ...}
            counList: {}, // 对应市的所有县区：{'130201': '市辖区', '130202': '路南区', '130203': '路北区', ...}
            streList: {}, // 对应县区的所有街道：{'130202001000': '学院南路街道', '130202002000': '友谊街道', ...}
            provIndex: 0,
            cityIndex: 0,
            counIndex: 0,
            streIndex: 0,
    ...
    watch: {
        ...
        value () {
            this.init();
            // 定制化程序，针对目前采用的三级地区结构,用以触发下拉菜单保持与value同步更新
            if (this.value[1]) {
                // window.alert(this.currCit + areaData[this.value[0]][this.value[1]]);
                this.currCit = areaData[this.value[0]][this.value[1]];
            }
            if (this.value[2]) {
                // window.alert(this.currCou + areaData[this.value[1]][this.value[2]]);
                this.currCou = areaData[this.value[1]][this.value[2]];
            }
        }
    },
    methods: {
        ...
        updateNextSelector (index, list, nextList, name, nextName, level) {
            if (level <= this.showLevel) {
                ...
                if (this.isInit && this.value[level]) {
                    ...
                    if (isNaN(parseInt(valueItem))) { // data-type数据格式不为"code"时
                        if (util.getIndex(this[list], this.value[level])) {
                            name = valueItem;
                        }
                    } else {
                        if (Object.keys(this[list]).indexOf(this.value[level]) > -1) { // data-type数据格式为"code"时
                            if (level === 0) {
